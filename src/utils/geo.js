...
// ✅ ZIP code utilities including ZIP → State/City lookup
// ============================================================
//
// CURRENT APPROACH: 3-digit ZIP prefix mapping
// - Covers ~91% of US ZIP codes accurately
// - Works offline, no API needed
// - Some edge cases may be incorrect (e.g., cross-state ZIPs)
//
// ⚠️ LONG-TERM UPGRADE OPTIONS (pick one):
//
// OPTION A: Full ZIP dataset JSON (~300KB gzipped)
//   - Download from: https://www.unitedstateszipcodes.org/zip-code-database/
//   - 100% accurate, includes city names
//   - Load async: const zipData = await import('./zip-database.json')
//
// OPTION B: Backend API endpoint
//   - GET /api/geo/zip/:zip → { state, city, county }
//   - Use USPS API or commercial provider (SmartyStreets, etc.)
//   - Most flexible, always up-to-date
//
// OPTION C: NPM package (e.g., 'zipcodes' or 'us-zips')
//   - npm install zipcodes
//   - import zipcodes from 'zipcodes'
//   - zipcodes.lookup('10001') → { state: 'NY', city: 'New York', ... }
//
// For now, prefix mapping works for MVP. Upgrade when needed.
// ============================================================

// ZIP code prefix to state mapping (first 3 digits)
// This covers all US ZIP codes without needing an API call
const ZIP_PREFIX_TO_STATE = {
  ...
  // New York
  '005': 'NY', // 00501-00544 IRS Holtsville, NY
  // Puerto Rico (006-009) - NOT included, returns null for non-continental
  ...
  // Alaska (not continental, but included for completeness)
  '995': 'AK', '996': 'AK', '997': 'AK', '998': 'AK', '999': 'AK',
  // Hawaii (not continental, but included for completeness)
  '967': 'HI', '968': 'HI',
};

/**
 * Look up state from ZIP code (synchronous, uses prefix mapping)
...
/**
 * Validate if a ZIP code format is correct
...
/**
 * Check if ZIP code is in continental US (48 states + DC)
 * @param {string} zip - 5-digit ZIP code
 * @returns {boolean}
 */
/**
 * Check if ZIP code is in continental US (lower 48 states + DC only)
 * Explicitly excludes:
 * - AK (Alaska)
 * - HI (Hawaii)
 * - PR (Puerto Rico) - returns null from mapping anyway
 * - GU (Guam) - returns null from mapping anyway
 * - VI (US Virgin Islands) - returns null from mapping anyway
 * - AS (American Samoa) - returns null from mapping anyway
 * - MP (Northern Mariana Islands) - returns null from mapping anyway
 * 
 * Note: DC is included as it's geographically within the continental US
 * 
 * @param {string} zip - 5-digit ZIP code
 * @returns {boolean} - true if ZIP is in lower 48 + DC
 */
export function isContinentalUSZip(zip) {
  const state = getStateFromZip(zip);
  if (!state) return false; // Unknown, territories, military APO/FPO all return false
  
  // Explicit exclusion list for non-continental states/territories
  // Even though territories return null from getStateFromZip(), we list them
  // here for documentation and future-proofing if mapping expands
  const nonContinental = new Set([
    'AK',  // Alaska
    'HI',  // Hawaii
    'PR',  // Puerto Rico
    'GU',  // Guam
  ...
  return !nonContinental.has(state);
}

/**
 * Get ZIP info including state (async version for future API integration)
 * @param {string} zip - 5-digit ZIP code
 * @returns {Promise<{state: string, city?: string} | null>}
 */
export async function lookupZipInfo(zip) {
  // For now, use the synchronous lookup
  // In the future, this could call a backend API for city info
  const state = getStateFromZip(zip);
  ...
  return {
    state,
    city: null, // City lookup would require an API or larger dataset
  };
}

// Legacy exports from original geo.js
const ZIP_COORDINATES = {
  '60601': { lat: 41.8857, lng: -87.6226, city: 'Chicago', state: 'IL' },
  '60602': { lat: 41.8827, lng: -87.6290, city: 'Chicago', state: 'IL' },
  '60290': { lat: 42.0447, lng: -88.0158, city: 'Schaumburg', state: 'IL' },
  '46204': { lat: 39.7776, lng: -86.1458, city: 'Indianapolis', state: 'IN' },
  ...
  '38103': { lat: 35.1495, lng: -90.0490, city: 'Memphis', state: 'TN' }
};
...
export const isZipInRadius = async (centerZip, targetZip, radiusMiles) => {
  try {
    const centerCoords = await getZipCoordinates(centerZip);
    const targetCoords = await getZipCoordinates(targetZip);
    ...
    console.error('Error checking ZIP radius:', error);
    return false;
  }
};
